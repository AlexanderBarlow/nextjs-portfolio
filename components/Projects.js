...
function Projects() {
  return (
    <section
      className="lg:grid lg:grid-rows-3 lg:grid-flow-col lg:gap-4 lg:items-center lg:h-screen lg:snap-center grid grid-rows-3 grid-flow-col items-center h-screen snap-start"
      id="projects"
    >
      <a href="projects"></a>
      <div className="lg:w-60 lg:row-start-1 lg:row-span-1 row start-1 row-span-1" id="lottie1">
        <h1 className="text-white text-6xl underline">Projects</h1>
      </div>
      <div className="lg:row-end-4 lg:row-span-3">
        <div className="flex space-x-4">
          {projects.map((item) => (
            <div
              className={classNames(
                "text-gray-300 lg:hover:bg-gray-700 hover:text-white",
                "lg:rounded-md lg:px-8 lg:py-8 lg:text-sm lg:font-medium ",
                ""
              )}
            > <a href={item.deployment}>
              <h1 key={item.name} className="text-purple-700 text-3xl">
                {item.name}
              </h1>
              <p className="lg:text-xl">{item.description}</p>
              <ul className="">
                <li className="bg-purple-950 text-purple-400 font-semibold  lg:py-2 lg:px-4 py-1 px-1 border border-purple-500  rounded ">{item.skill1}</li>
                <li className="bg-purple-950  text-purple-400 font-semibold  lg:py-2 lg:px-4 py-1 px-1 border border-purple-500  rounded">{item.skill2}</li>
              </ul>
              </a>
            </div>
